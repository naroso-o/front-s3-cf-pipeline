...
- CloudFront 배포 도메인 이름: https://d2xm35e5plqmno.cloudfront.net

## 성능 최적화

S3 엔드포인트를 통한 접속과 CloudFront 배포 도메인을 통한 접속을 비교해보았습니다. Chrome 브라우저에서 테스트를 진행했습니다.

### 1. 초기 로드 속도 테스트

브라우저 2개를 열고, 개발자 도구를 미리 열어둔 상태에서 각각 S3와 CloudFront에 동시에 접속하여 로드 속도를 비교합니다.

![초기 로드 이미지](/public/readmd/initial_load.png)

**S3 직접 접속 (좌측)**:

- 전체 로드 완료 시간: **약 71ms**
- HTML 문서 로드: 46ms
- 주요 리소스들(CSS, JS, 이미지)의 로드 시간이 상대적으로 깁니다.
- 각 파일별로 30-70ms 범위의 로드 시간을 보입니다.

**CloudFront 배포 (우측)**:

- 전체 로드 완료 시간: **약 25ms**
- HTML 문서 로드: 89ms (초기 접속은 다소 오래 걸렸지만)
- 대부분의 리소스가 17-25ms 내에 로드 완료됩니다.
- 특히 정적 파일들(CSS, JS, 이미지)의 로드 속도가 현저히 빠릅니다.

### 2. 캐시 효과 테스트

페이지를 새로고침하여 캐시된 리소스의 로드 속도를 비교합니다. CloudFront의 엣지 캐싱 효과를 확인할 수 있습니다.

![캐시 로드 이미지](/public/readmd/cached_load.png)

**S3 직접 접속 (좌측)**:

- 전체 로드 완료 시간: **약 130ms**
- HTML 문서 로드: 77ms
- 대부분의 리소스가 35-130ms 범위의 로드 시간을 보입니다.
- 캐시 효과가 제한적이며, 여전히 상당한 로드 시간이 소요됩니다.

**CloudFront 배포 (우측)**:

- 전체 로드 완료 시간: **약 23ms**
- HTML 문서 로드: 118ms (초기 접속)
- 대부분의 정적 리소스가 11-23ms 내에 로드 완료됩니다.
- 캐시된 파일들의 로드 속도가 현저히 개선되었습니다.

### +) Chrome 시크릿모드 테스트

CloudFront는 87ms만에 응답을 받았는데, S3는 정상 화면 대신 '보안 연결을 지원하지 않습니다' 라는 오류 문구만 표시됩니다.

![시크릿모드 이미지](/public/readmd/secret.png)
S3 웹사이트 엔드포인트는 HTTP만 지원하지만, CloudFront는 HTTPS를 기본으로 제공합니다. 최신 브라우저들은 보안상의 이유로 HTTPS 연결을 우선시하므로, S3 직접 접속 시 보안 경고가 발생할 수 있습니다.

### 결론

| 테스트 항목          | S3 직접 접속              | CloudFront 배포            | 성능 개선률     |
| -------------------- | ------------------------- | -------------------------- | --------------- |
| **초기 로드 속도**   | 약 71ms                   | 약 25ms                    | **65% 개선**    |
| **캐시된 로드 속도** | 약 130ms                  | 약 23ms                    | **82% 개선**    |
| **HTML 문서 로드**   | 46ms (초기) / 77ms (캐시) | 89ms (초기) / 118ms (캐시) | 일부 차이       |
| **정적 리소스 로드** | 30-130ms                  | 11-25ms                    | **60-80% 개선** |
